...
function wireCompanion() {
  try {
    state.waifu = new Waifu.Companion($('waifuCanvas'));
    state.waifu.start();
  } catch (err) {
    $('stage').innerHTML =
      `<div class="empty">3D companion unavailable: ${escapeHtml(err.message)}</div>`;
    return;
  }
  state.waifu.onSpeechEnd = () => { $('btnStopSpeech').hidden = true; };

  $('charSelect').addEventListener('change', () => selectCharacter($('charSelect').value));
  $('charEmotion').addEventListener('change', () => {
    state.waifu.setExpression($('charEmotion').value);
    $('stageBadge').textContent = $('charEmotion').value;
  });
  ...
  $('btnIdle').addEventListener('click', async () => {
    ...
    try {
      const line = await postJSON(`/api/companion/characters/${state.character.id}/idle`);
  ...
  $('btnSay').addEventListener('click', async () => {
    ...
    btn.disabled = true;
    try {
      const line = await postJSON(`/api/companion/characters/${state.character.id}/say`,
                                  { text, emotion: $('charEmotion').value });
      await performLine(line);
    } catch (err) { toast(err.message, true); }
    finally { btn.disabled = false; }
  });
  ...
  $('chatForm').addEventListener('submit', async (ev) => {
    ...
    btn.disabled = true;
    try {
      const reply = await postJSON(`/api/companion/characters/${state.character.id}/chat`,
                                   { message: text, speak: true });
      appendBubble('assistant', reply.reply, reply.emotion, reply);
      await performLine(reply);
      $('chatEngineHint').textContent = reply.engine === 'llm'
        ? 'Replies generated by the configured LLM.'
        : 'Scripted persona replies — set OPENAI_API_KEY for open-ended conversation.';
    } catch (err) { toast(err.message, true); }
    finally { btn.disabled = false; }
  });
  ...
  $('btnClearChat').addEventListener('click', async () => {
    if (!state.character) return;
    await api(`/api/companion/characters/${state.character.id}/history`, { method: 'DELETE' });
    $('chatLog').innerHTML = '';
    toast('Conversation cleared');
  });

  // appearance controls drive the renderer live, and save on demand
  const appearanceInputs = {
    hair_style: 'apHairStyle', outfit_style: 'apOutfitStyle', hair_color: 'apHairColor',
    eye_color: 'apEyeColor',
    skin_color: 'apSkinColor', outfit_color: 'apOutfitColor', accent_color: 'apAccentColor',
    eye_size: 'apEyeSize', blush: 'apBlush', height: 'apHeight',
  };
  Object.entries(appearanceInputs).forEach(([key, id]) => {
    $(id).addEventListener('input', () => {
      const el = $(id);
      const value = el.type === 'range' ? Number(el.value) : el.value;
      state.waifu.setAppearance({ [key]: value });
  ...
  $('apVoice').addEventListener('change', async () => {
    if (!state.character) return;
    try {
      state.character = await patchJSON(
        `/api/companion/characters/${state.character.id}`, { voice_id: $('apVoice').value });
      toast('Voice updated');
    } catch (err) { toast(err.message, true); }
  });
  ...
  $('btnSaveAppearance').addEventListener('click', async () => {
    ...
    try {
      state.character = await patchJSON(
        `/api/companion/characters/${state.character.id}`, { appearance });
      $('appearanceMsg').className = 'msg ok';
      $('appearanceMsg').textContent = 'Appearance saved.';
      await loadCharacters(state.character.id);
    } catch (err) {
      $('appearanceMsg').className = 'msg error';
      $('appearanceMsg').textContent = err.message;
    }
  });

  $('btnDeleteChar').addEventListener('click', async () => {
    if (!state.character) return;
    if (!confirm(`Delete "${state.character.name}"?`)) return;
    try {
      await api(`/api/companion/characters/${state.character.id}`, { method: 'DELETE' });
      await loadCharacters();
      toast('Companion deleted');
    } catch (err) { toast(err.message, true); }
  });

  $('btnCreateChar').addEventListener('click', async () => {
    const name = $('newCharName').value.trim();
    if (!name) return toast('Give your companion a name', true);
    try {
      const created = await postJSON('/api/companion/characters', {
        name,
        voice_id: $('newCharVoice').value,
        persona: $('newCharPersona').value.trim(),
        greeting: $('newCharGreeting').value.trim(),
        appearance: state.waifu.appearance,
      });
      $('newCharMsg').className = 'msg ok';
      $('newCharMsg').textContent = `Created "${created.name}".`;
      await loadCharacters(created.id);
    } catch (err) {
      $('newCharMsg').className = 'msg error';
      $('newCharMsg').textContent = err.message;
    }
  });
}

async function performLine(line) {
  if (!state.waifu || !line || !line.audio) return;
  $('bubble').hidden = false;
  $('bubble').textContent = line.text || line.reply || '';
  $('stageBadge').textContent = line.emotion || 'neutral';
  $('charEmotion').value = line.emotion || 'neutral';
  $('btnStopSpeech').hidden = false;
  try {
    await state.waifu.speak(line);
  } catch (err) {
    $('btnStopSpeech').hidden = true;
    toast('Audio playback blocked — click the page once, then try again', true);
  }
}

function appendBubble(role, text, emotion, line) {
  const log = $('chatLog');
  const el = document.createElement('div');
  el.className = `bubble ${role}`;
  const tag = document.createElement('span');
  tag.className = 'tag';
  tag.textContent = role === 'user' ? 'you' : (emotion || 'reply');
  el.appendChild(tag);
  el.appendChild(document.createTextNode(text));
  if (role === 'assistant' && line && line.audio) {
    const replay = document.createElement('button');
    replay.className = 'replay';
    replay.textContent = '▶ replay';
    replay.addEventListener('click', () => performLine({ ...line, text }));
...
async function loadCharacters(selectId) {
  state.characters = await getJSON('/api/companion/characters');
  const sel = $('charSelect');
  sel.innerHTML = state.characters.map((c) =>
    `<option value="${c.id}">${escapeHtml(c.name)}</option>`).join('');
  const emotions = state.system.emotions || ['neutral'];
  $('charEmotion').innerHTML = emotions.map((e) =>
    `<option value="${e}">${e}</option>`).join('');

  const target = selectId && state.characters.some((c) => c.id === selectId)
    ? selectId : (state.characters[0] && state.characters[0].id);
  if (target) {
    sel.value = target;
...
async function selectCharacter(characterId) {
  ...
  if (!character || !state.waifu) return;
  state.character = character;

  $('charName').textContent = character.name;
  state.waifu.setAppearance(character.appearance);
  state.waifu.setExpression(character.default_emotion || 'neutral');
  $('charEmotion').value = character.default_emotion || 'neutral';
  $('stageBadge').textContent = character.default_emotion || 'neutral';

  const a = character.appearance || {};
  $('apHairStyle').value = a.hair_style || 'twintails';
  $('apOutfitStyle').value = a.outfit_style || 'dress';
  $('apHairColor').value = a.hair_color || '#f2739d';
  $('apEyeColor').value = a.eye_color || '#4fc3f7';
  $('apSkinColor').value = a.skin_color || '#ffe0d0';
  $('apOutfitColor').value = a.outfit_color || '#3d4b7a';
  $('apAccentColor').value = a.accent_color || '#ffffff';
  [['apEyeSize', a.eye_size ?? 1], ['apBlush', a.blush ?? 0.35],
   ['apHeight', a.height ?? 1]].forEach(([id, v]) => {
    $(id).value = v;
    if ($(id).nextElementSibling) $(id).nextElementSibling.textContent = Number(v).toFixed(2);
  });
  $('apVoice').value = character.voice_id;
  $('btnDeleteChar').hidden = character.owner === 'system';

  $('chatLog').innerHTML = '';
  if (character.greeting) appendBubble('assistant', character.greeting, 'greeting');
  const history = await getJSON(`/api/companion/characters/${characterId}/history`);
  history.forEach((turn) => appendBubble(turn.role, turn.text, turn.emotion));
  $('bubble').hidden = true;
}

function fillCompanionVoiceSelects() {
  const options = state.voices.map((v) =>
    `<option value="${v.id}">${escapeHtml(v.name)}</option>`).join('');
  ['apVoice', 'newCharVoice'].forEach((id) => {
    const el = $(id);
    if (!el) return;
